...
class intervalExpension {
  ...
  _createIntervalsFromBoundaries(intervalBoundaries) {
    ...
    for (
      let i = 0, j = 1;
      i < boundariesCount - 1, j < boundariesCount;
      i++, j++
    ) {
  ...
  _sortIntervals(intervals) {
    intervals.sort((x, y) => {
      ...
      return (
        this.monthsMap.get(month1.toLowerCase()) -
        this.monthsMap.get(month2.toLowerCase())
      );
    });
    return intervals;
  }
...
// console.log(
//   `Case ${EnteredCase}: ${intervalSimplifier.unfoldIntervals(
//     intervals,
//     EnteredCase
//   )}`
// );
